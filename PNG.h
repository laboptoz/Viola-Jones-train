...
public:
    //template <class Type>
    PNG(std::string &fileName, int height, int width) : Picture(fileName, height, width) {
        size.height = height;
        size.width = width;
    }
    ...
    void load(){
        ...
        //	image = cvCreateImage(size, src->depth, src->nChannels);
        //cvResize(src, image, 0);
        w = (size_t)image->width;
        h = (size_t)image->height;
        //cvReleaseImage(&src);
